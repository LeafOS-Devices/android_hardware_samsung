...
static constexpr const char* kVibratorPropPrefix = "ro.vendor.vibrator_hal.";
static constexpr const char* kVibratorPropDuration = "_duration";
...
static int getProperty(std::string key, int def) {
...
uint32_t Vibrator::effectToMs(Effect effect, ndk::ScopedAStatus* status) {
    *status = ndk::ScopedAStatus::ok();
    switch (effect) {
        case Effect::CLICK:
            return getProperty("click" + kVibratorPropDuration, 10);
        case Effect::TICK:
            return getProperty("tick" + kVibratorPropDuration, 5);
        case Effect::TEXTURE_TICK:
            return getProperty("texture_tick" + kVibratorPropDuration, 5);
        default:
            break;
    }
    *status = ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
